...
iedit.nvim/
├── lua/iedit/
│   ├── init.lua         # Main module - multiple cursor functionality
│   ├── config.lua       # Configuration management
│   ├── logger.lua       # Logging (optional, depends on logger.nvim)
...
- **Module name:** `iedit`
- **Entry point:** `require('iedit')` → `lua/iedit/init.lua`
- **API:** `require('iedit').setup(opts)`, `require('iedit').start()`
- **Optional dependency:** `logger.nvim` (for debug logging)
- **LuaRocks module:** `wsdjeg/iedit.nvim`
